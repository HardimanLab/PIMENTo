...
#' @usage preprocessPlots(inputFile, fileSheet=1, ntext=2, dataCol, name=1, id=2)
...
#' @param name Column index which contains gene names
#' @param id Column index which contains gene ID's
...
preprocessData <- function(inputFile,fileSheet=1,ntext=2,dataCol,name=1,id=2) {

  ext <- tools::file_ext(inputFile)
  if (ext=="xlsx")
    X <- openxlsx::read.xlsx(inputFile,sheet=fileSheet)
  else if (ext=="csv")
    X <- read.csv(inputFile,header=TRUE,sep=",")
  else
    stop("Input file must be a .xlsx spreadsheet or .csv")
